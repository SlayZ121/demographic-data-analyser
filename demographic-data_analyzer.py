...
def calculate_demographic_data(print_data=True):
    ...
    male_df=df[df['education']=='Bachelors']
    percentage_bachelors = round((len(male_df)/len(df))*100,1)
    ...
    rich_percentage = round((len(num_min_workers) / len(df[df['hours-per-week'] == min_work_hours])) * 100,1)
    ...
    top_IN_occupation = high_earning_india['occupation'].mode()[0]
